...
export const useCreateAutomation = () => {
  ...
  return { isPending, mutate, mutationId };
};
...
export const useEditAutomation = (automationId: string) => {
  ...
  const { isPending, mutate } = useMutationData(
    ["update-automation"],
    (data: { name: string }) =>
      AutomationService.updateAutomation(automationId, { name: data.name }),
    "automation-info",
    disableEdit
  );
...
export const useListener = (id: string) => {
  ...
  const { isPending, mutate } = useMutationData(
    ...
    (data: { prompt: string; reply: string }) =>
      AutomationService.saveListener(
        id,
        listener || "MESSAGE",
        data.prompt,
        data.reply
...
export const useTriggers = (id: string) => {
  ...
  const { isPending, mutate } = useMutationData(
    ["add-trigger"],
    (data: { types: string[] }) =>
      AutomationService.saveTrigger(id, data.types),
    "automation-info"
  );
...
export const useKeywords = (id: string) => {
  ...
  const { mutate } = useMutationData(
    ["add-keyword"],
    (data: { keyword: string }) =>
      AutomationService.saveKeyword(id, data.keyword),
    "automation-info",
    () => setKeyword("")
  );
  ...
  const { mutate: deleteMutation } = useMutationData(
    ["delete-keyword"],
    (data: { id: string }) => AutomationService.deleteKeyword(id, data.id),
    "automation-info"
  );
